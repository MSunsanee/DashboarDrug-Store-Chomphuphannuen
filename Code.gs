...
function updateMasterLabelInfo_(p){
  const lock=LockService.getDocumentLock();lock.waitLock(30000);
  try{
    const list=master_(),item=list.find(x=>x.key===String(p.key||''));if(!item)throw new Error('ไม่พบรายการยา');
    const sh=SpreadsheetApp.getActive().getSheetByName(SHEETS.MASTER),row=list.indexOf(item)+2,current=sh.getRange(row,10,1,4).getDisplayValues()[0];
    const next=[p.thaiName,p.indication,p.directions,p.warning].map((value,i)=>String(value||'').trim()||String(current[i]||''));
    if(!next.some(Boolean))throw new Error('กรุณากรอกข้อมูลฉลากอย่างน้อย 1 ช่อง');
    sh.getRange(row,10,1,4).setValues([next]);
    return{key:item.key,thaiName:next[0],indication:next[1],directions:next[2],warning:next[3]};
  }finally{lock.releaseLock()}
}
...
function authorizeAction_(action,token,p){
  ...
  if(action==='updateMasterLabelInfo'){
    if(user.role==='viewer')throw new Error('บัญชีเยี่ยมชมไม่สามารถแก้ไขฐานข้อมูลฉลากยาได้');
    return;
  }
  const masterActions=new Set(['createMaster','updateMaster','bulkCreateMaster','deleteMaster','updateMasterStockType','updateMasterUsage','updateMasterUnit']);
